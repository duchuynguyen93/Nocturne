...
presentation, with `vo=libmpv` and `gpu-next` (libplacebo).

## Consequences

Positive:

- libplacebo's renderer, immediately: `ewa_lanczossharp` scaling, debanding,
...
free — genuinely simpler than the design in ADR 0002 — but has no libplacebo, no
...
renderer is not in libplacebo's class, and its Windows embedding path leads back
